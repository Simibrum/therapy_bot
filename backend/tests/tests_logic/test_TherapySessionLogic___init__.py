...
from backend.logic.therapy_session_logic import (
    TherapySessionLogic, TherapistDoesNotExistError,
    TherapySessionDoesNotExistError, UserIdMismatchError
)
...
class Test__Init__:
    ...
    def test_init_with_non_existent_pre_existing_session_id(self, therapy_session_instance):
        with pytest.raises(TherapySessionDoesNotExistError):
            session = TherapySessionLogic(pre_existing_session_id=999)
    ...
    def test_init_with_non_matching_user_id(self, therapy_session_instance):
        with pytest.raises(UserIdMismatchError):
            session = TherapySessionLogic(user_id=999, pre_existing_session_id=therapy_session_instance.id)
