...
def compute_cosine_similarities(query_embedding: np.array, embeddings_matrix: np.array) -> np.array:
    ...
    if not isinstance(query_embedding, np.ndarray) or not isinstance(embeddings_matrix, np.ndarray):
        msg = "Input arrays must be numpy arrays."
        raise TypeError(msg)
    ...
    if query_embedding.shape[0] != embeddings_matrix.shape[1]:
        ...
        raise ValueError(msg)
    if (
        np.isnan(query_embedding).any()
        or np.isnan(embeddings_matrix).any()
        or np.isinf(query_embedding).any()
        or np.isinf(embeddings_matrix).any()
    ):
        msg = "Input arrays contain NaN or infinite values"
        raise ValueError(msg)

    return np.dot(query_embedding.T, embeddings_matrix.T)
